...
def _load_snapshot(store: MoegirlKnowledgeStore, status: LocalEmbeddingStatus) -> VectorIndexSnapshot:
    ...
    for row in rows:
        ...
        if dimensions != status.dimensions or not isinstance(raw, bytes) or len(raw) != dimensions * 2:
            continue
    ...
    *,
    ...
    for index in np.argsort(-scores):
        ...
        if entry_key(entry) in disabled:
            continue
        if allowed_source_tags is not None and entry.source_tag not in allowed_source_tags:
            continue
        key = entry_key(entry)
...
async def semantic_search(
    ...
    *,
    ...
    try:
        vector = await asyncio.wait_for(service.embed(query), timeout=1.0)
    except Exception:
...
async def index_embedding_batch(
    ...
    *,
    ...
    chunks = store.pending_embedding_chunks(model_id=status.model_id, limit=batch_size)
    if not chunks:
        return 0
    texts = [str(chunk["text"]) for chunk in chunks]
    try:
        vectors = await asyncio.wait_for(service.embed_batch(texts), timeout=15.0)
    ...
    with mutation_lock(store.database_path):
        for index, chunk in enumerate(chunks):
            ...
            stored += int(store.store_chunk_embedding(
                chunk_id=str(chunk["chunk_id"]),
                content_hash=str(chunk["content_hash"]),
