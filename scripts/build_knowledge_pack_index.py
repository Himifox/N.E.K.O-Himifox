...
from knowledge.subscriptions import load_canonical_pack_artifact
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("pack", type=Path, help="canonical .neko-knowledge.json artifact")
    parser.add_argument("--output-dir", type=Path, help="directory for generated sidecars")
...
def _derived_chunks(pack_raw: bytes):
...
async def _build(pack_path: Path, output_dir: Path) -> dict[str, object]:
    ...
    chunks = _derived_chunks(pack_raw)
    ...
    try:
        ...
        if service.model_id() != PREBUILT_MODEL_ID or int(service.dim() or 0) != PREBUILT_DIMENSIONS:
            raise RuntimeError("local embedding model does not match the publication contract")
        records: list[dict[str, object]] = []
        for offset in range(0, len(chunks), MICROBATCH_SIZE):
            batch = chunks[offset:offset + MICROBATCH_SIZE]
            vectors = await service.embed_batch([chunk.embedding_text for chunk in batch])
            if len(vectors) != len(batch):
                raise RuntimeError("embedding batch returned an unexpected result count")
            for chunk, vector in zip(batch, vectors, strict=True):
                array = np.asarray(vector, dtype=np.float32)
                if array.shape != (PREBUILT_DIMENSIONS,) or not bool(np.isfinite(array).all()):
...
def _verify(pack_path: Path, manifest_path: Path, vectors_path: Path) -> dict[str, object]:
    pack_raw = pack_path.read_bytes()
