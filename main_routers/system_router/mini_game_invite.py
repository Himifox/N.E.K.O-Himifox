...
def _mini_game_invite_get_state(lanlan_name: str) -> dict[str, Any]:
    ...
    if state is None:
        ...
        state.setdefault('pending_turn_id', None)
    return state
...
def _mini_game_invite_record_delivered(
    lanlan_name: str,
    session_id: str,
    *,
...
def _apply_mini_game_invite_choice(
    ...
    if choice == 'accept':
        ...
        invite_turn_id = state.get('pending_turn_id') or ''
    ...
    if choice == 'decline':
        ...
        decline_turn_id = state.get('pending_turn_id') or ''
        game_type = state.get('last_game_type') or 'soccer'
        state['responded_at'] = now
        state['chats_since_response'] = 0
    ...
    if choice == 'later':
        ...
        later_turn_id = state.get('pending_turn_id') or ''
        ...
        return {
            ...
            'game_type': state.get('last_game_type') or '',
        }
    return {'action': 'ignored', 'reason': f'unknown_choice:{choice}'}


def _record_mini_game_invite_feedback(
    lanlan_name: str,
    choice: str,
    ...
    *,
    ...
    if source == "implicit_dismiss":
        return
    turn_id = str(result.get("turn_id") or "").strip()
    if not turn_id:
        return
    event_type_by_choice = {
        "accept": "mini_game_accept",
        "later": "mini_game_later",
        "decline": "mini_game_decline",
    }
    event_type = event_type_by_choice.get(choice)
    if not event_type:
        return
    try:
        record_feedback_event(
        ...
        logger.debug("[%s] mini-game feedback record failed: %s", lanlan_name, exc)
...
async def mini_game_invite_respond(request: Request):
    ...
    _record_mini_game_invite_feedback(lanlan_name, choice, result, source="button")
...
def _maybe_apply_mini_game_invite_keyword(
    ...
    if result.get('action') == 'ignored':
        return None
    _record_mini_game_invite_feedback(lanlan_name, choice, result, source="keyword")
    return result
