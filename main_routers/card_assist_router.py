...
# 这些情态词只服务于卡片编辑疑问守卫，放在消费它们的模块里，避免卡片逻辑
# 反向依赖音乐点歌实现。
_CHAT_A_NOT_A_MODALS = (
...
def _chat_a_not_a_forms() -> list[str]:
    forms: list[str] = []
    for modal in _CHAT_A_NOT_A_MODALS:
...
_CHAT_QUESTION_CLAUSE_RE = re.compile(
    ...
    + r"|(?<!所)(?:" + "|".join(_chat_a_not_a_forms()) + r")"
