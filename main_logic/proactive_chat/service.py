...
async def handle_proactive_chat(
    ...
    *,
    ...
    try:
        ...
        recommendation_turn.activity_snapshot = activity_snapshot
        ...
        source_weights: dict[str, float] = dict(source_weight_selection.weights)
        ...
        try:
            recommendation_turn.decide_source(
                enabled_modes=enabled_modes,
            ...
            logger.debug(
                "[%s] proactive recommendation source decision failed: %s",
                lanlan_name,
                exc,
            )
        ...
        try:
            phase1_topics = recommendation_turn.decide_material(
                enabled_modes=enabled_modes,
                source_weights=source_weights,
            ...
            logger.debug(
                "[%s] proactive recommendation material decision failed: %s",
                lanlan_name,
                exc,
            )
        ...
        recommendation_turn.delivered_text = response_text
        return await _end_proactive(
            ProactiveChatResult(
