...
内建 Core 把浏览器扩展行为和内容画像保存在 OpenBiliClaw 自己的本地目录。主动预览
不会把完整画像、原始浏览/收藏/稍后看记录发送给 NEKO 模型。Core 可在本机内部预览
最多 3 条，但适配层只把排名第 1 条交给 Phase 1，OBC 最多占 1 个槽位；Phase 2 只
看到最终一条的标题、主题、摘要和选题动机。URL 与投递身份
只留在代码层。
Core 的摘要可靠度分量只用于内部门禁，不进入 NEKO Prompt，也不与 NEKO 记忆合并。
后台每日预算只读取 caller 与 Token 数量账本，不额外保存 Prompt、候选正文、画像文本或
模型凭据。

健康、金融、政治或宗教候选的门禁最多读取活动会话内存中的最近三条用户消息，判断
用户是否明确提起同一主题；交接层不持久化这些消息，也不发送给模型。仅由浏览推断的
敏感兴趣会被拒绝；诊疗、投资买卖建议、政治劝服和信仰推断始终拒绝。
