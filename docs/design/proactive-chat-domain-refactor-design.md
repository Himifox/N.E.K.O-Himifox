...
1. `codex/proactive-chat-domain-foundation`：迁移契约、状态、决策、生成保护、小游戏邀请和音乐推荐；Router 仍保留完整编排，旧入口可以独立运行。
2. `codex/proactive-chat-service-orchestration`：在第一阶段之上迁移 source、delivery、提醒和主 Service 编排，将 Router 收缩为协议适配与兼容门面。

第二个 PR 以第一个 PR 为审核基线；第一个 PR 合入 `main` 后再重定向和 rebase。回滚第二个 PR 会恢复到仍可运行的 Router 编排状态；回滚第一个 PR 则恢复重构前实现。任何阶段都不通过复制状态实现临时双写。

前端内部拆分仍独立排期，不纳入这两个 PR。
