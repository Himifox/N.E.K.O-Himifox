...
状态：设计修订定稿。本文基于 2026-07-15 的仓库结构，定义主动搭话领域模块化重构的目标结构、职责边界、迁移路线、兼容策略和验收标准，不代表代码已经完成迁移。
...
原单体 `main_routers/system_router.py` 已经完成第一轮按路由领域拆包，当前主动搭话相关实现主要分布在：
...
```text
main_routers/system_router/
  proactive_chat_flow.py     # 过渡期兼容门面；最终只保留主动搭话 HTTP 路由
  mini_game_invite.py        # 最终只保留邀请反馈 HTTP 路由和协议适配
  __init__.py                # 临时 re-export；迁移完成后逐步收缩
```
...
Router adapters
  -> proactive_chat.service
    -> contracts / state / decisions / generation
...
| `proactive_content.py` 中纯日志辅助 | 保留或下沉到对应领域模块 | 不得记录原始隐私对话到 logger。 |
...
| `break_reminders.py` | 原位保留或作为明确外部 collaborator | 本轮不强行并入主动搭话包；service 通过窄接口调用。 |
...
- 旧 Router 模块只能 re-export 同一个对象或函数，不得复制一份字典、deque、锁或 loaded flag。
...
- 目标测试、Node 测试、项目检查和 `git diff --check` 全部通过。
...
| 职责边界 | Router 子模块仍同时承担 HTTP、领域状态、策略和生成编排。 | Router、领域逻辑、通用工具、Prompt 和投递状态机各自归位。 |
| 状态所有权 | 历史和 source 模块之间存在对可变全局状态的直接引用。 | `state.py` 是唯一所有者，其它模块通过接口读取和更新。 |
| 子能力规模 | 小游戏路由、状态、业务规则和 WebSocket payload 集中在同一 Router 文件。 | 小游戏和音乐拥有独立领域模块，Router 只做协议适配。 |
| 测试方式 | 部分测试仍依赖 Router 内部函数或兼容门面。 | 领域规则直接单测，Router 只保留少量协议契约测试。 |
| 行为稳定性 | 大流程新增分支时容易遗漏 reason、stage、历史、计数或生命周期清理。 | 契约、状态、投递提交和生命周期结束均由明确边界统一处理。 |
