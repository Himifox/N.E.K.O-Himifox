...
> 状态：远期候选，`HOLD`。当前固定阈值仍属于基线事实，本阶段不改反馈判定或持久化结构。
...
> 状态：远期候选，`HOLD`。本节只规定隐私原则，不授权新增持久画像。
...
> 状态：远期候选，`HOLD`。当前保留原始 feedback event 和既有奖励逻辑，不实现 `reward_score_v2`。
...
|---|---|---|
| 个人回复时延基线 | 不改固定 60 秒反馈基线 | 独立设计、隐私状态模型、Golden 上的增量证据 |
| 临时/持久兴趣 | 不新增画像或 `reward_score_v2` | 明确 TTL/衰减/幂等/删除契约及离线非劣结果 |
