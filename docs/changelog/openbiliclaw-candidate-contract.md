...
- Phase 1 receives up to three bounded candidates and selects by displayed
  sequence number within the existing call.
...
are unchanged. N.E.K.O pins OpenBiliClaw Core commit `6f4c7aea`.

The contract is covered by prompt-boundary unit tests, single-speaker tests, and
a real-Core three-candidate integration test that confirms only the selected
