...
## Built-in OpenBiliClaw recommendation flow
...
Tracking data, URLs, free-form OpenBiliClaw expression, full profiles, and raw
behavior never enter Phase 1 or Phase 2 prompts. The active session's last three
user messages may be checked in memory for deterministic sensitive-topic gating;
